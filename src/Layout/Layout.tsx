  ...
  return (
    <div className="w-screen h-screen flex overflow-hidden bg-gray-50 p-3 gap-3 relative">
      <div className="fixed left-0 top-4 z-9999 border-r-11 border-gray-800 flex flex-col -translate-x-61.25 w-64 items-center gap-2 rounded-r-2xl bg-white py-4 pl-4 pr-5 shadow-lg shadow-gray-950 ring-1 ring-red-100 transition-transform duration-300 hover:translate-x-0 md:top-6">
        ...
        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Volume Control</span>
        <ElasticSlider
          defaultValue={volume}
          startingValue={0}
          maxValue={100}
      ...
      <aside className={`hidden md:flex flex-col h-full shrink-0 gap-3 transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"}`}>
      ...
      <div className={`fixed inset-0 z-50 md:hidden bg-black/40 transition-opacity duration-300 ${isMobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} onClick={() => setIsMobileOpen(false)}>
        <aside className="w-64 h-[calc(100vh-24px)] flex flex-col gap-3 bg-red-800 m-3 rounded-2xl p-2 shadow-xl" onClick={(e) => e.stopPropagation()}>
