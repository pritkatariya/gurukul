...
export default function StudentListGurukulArt() {
  ...
  const fetchGurukulArtData = async () => {
    try {
      ...
      const resUsers = await fetch(`${API_URL}/gurukul-art/onboarded-users`);
      ...
      <div className="w-full bg-white p-4 sm:p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 text-xs sm:text-sm font-semibold mt-1">
            Realtime database request configurations and onboarding dashboard for Gurukul Art.
          </p>
        </div>
      </div>
      ...
      <div className="w-full bg-white rounded-3xl shadow-sm border border-gray-100 p-3 sm:p-5 flex flex-col">
        {loading ? (
          ...
          <div className="text-gray-400 font-bold py-24 text-center uppercase text-xs tracking-wider">
            No dynamic {activeTab.toLowerCase()} entries found for Gurukul Art.
          </div>
        ) : (
        ...
        {onboardedStudents.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(() => {
              ...
              const head = onboardedStudents.find(isHead);
              const others = onboardedStudents.filter((s) => s !== head);
                ...
                <>
                      ...
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-4">
                          <div className="relative">
                            {head.image_url ? (
                              <img src={head.image_url} alt={head.name} className="h-16 w-16 rounded-full object-cover border-2 border-red-200 shadow-xl" />
                            ) : (
                        ...
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                          ...
                          <div className="rounded-3xl bg-white border border-red-100 p-3 shadow-sm">
                            ...
                            <p className="mt-2 text-sm font-black text-gray-800">Gurukul Art</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {others.map((student) => (
                    <div key={student.id} className="flex items-center gap-3 p-3 bg-emerald-50/20 border border-emerald-100/50 rounded-2xl">
                      {student.image_url ? (
                        <img src={student.image_url} alt={student.name} className="w-10 h-10 rounded-full object-cover border border-emerald-200" />
                      ) : (
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-bold">User: {student.username} | SUID: {student.suid} | Joined: {student.joined_date ? new Date(student.joined_date).toLocaleDateString("en-GB") : ""}</p>
                      </div>
                    </div>
                  ))}
                </>
